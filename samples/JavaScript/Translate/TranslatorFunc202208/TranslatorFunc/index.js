...
module.exports = async function (context, req) {
    ...
    let originalText = req.body[0].text.toString();
    let translateTo = req.body[0].language.toString();
    let translateFrom;
    let translatedText;
    ...
    options = {                
        method: 'POST',
        // url: endpoint + 'translate?api-version=3.0&to=' + translateTo.toString(),
        url: endpoint + 'translate?api-version=3.0&to=' + translateTo + ((!translateFrom) ? '' : '&from=' + translateFrom),
        headers: {
            'Ocp-Apim-Subscription-Key': key,
            'Content-Type': 'application/json'
        },
    ...
    context.res = {
        ...
        body: 
            {
                'translateFrom': (!detectResult) ? 'error' : translateFrom,
                'translateTo': translateTo,
                'originalText': originalText,
                'translatedText': (!translateResult) ? 'error' : translatedText
            }
    };
}